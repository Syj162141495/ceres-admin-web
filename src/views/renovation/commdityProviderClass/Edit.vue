      ...
      >
        <div slot-scope="{ node, data }" class="custom-tree-node">
          ...
          <div v-if="!isCheck" class="setting-box">
            ...
            >{{ data.classifyLevel | addTips }}</el-button>
            <el-button
              type="text"
              size="mini"
...
export default {
  ...
  methods: {
    ...
    async queryByClassifyID(classifyId) {
      ...
      const res = await getProviderClassById({ classifyId: classifyId })
      const resData = res.data

      if (resData) {
        resData.children =
          resData && resData.children && resData.children.map(this.treeFilter)
    ...
    async addGroup() {
      ...
      const res = await providerClassAdd({ classifies: params })
      if (res.code === '') {
    ...
    async updateGroup() {
      const params = this.treeData.map(this.treeFilter)
      const res = await providerClassUpdate({
        classifies: params,
