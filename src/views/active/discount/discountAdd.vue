...
export default {
  ...
  methods: {
    ...
    async getDetails() {
      this.title = this.info.discountId ? "修改折扣活动" : "新增折扣活动";
      const res = await getDiscountDetail({ discountId: this.info.discountId });
    ...
    save() {
      ...
      if (this.info.discountId) {
        params.details = params.promotionDetail;
        return editDiscountData(
          Object.assign(params, { discountId: this.info.discountId })
        ).then((res) => {
          if (res.code === "") {
            this.$message({
              message: "修改成功",
